Feature: Teacher can assign an offering to a class
  ...
  Scenario: Teacher can assign an investigation to a class
    Given the following simple investigations exist:
      | name               | user    |
      | A Test Investigation | teacher |
    And the investigation "A Test Investigation" is assigned to the class "My Class"
    Then the investigation named "A Test Investigation" should have "offerings_count" equal to "1"
  ...
  Scenario: All potential offerings are visible
    Given the following simple investigations exist:
      | name                 | user    |
      | A Test Investigation | teacher |
    And the investigation "A Test Investigation" is published
    And the following resource pages exist:
      | name               | user    |
      | Test Resource Page | teacher |
    And I am logged in with the username teacher
    And I am on the class page for "My Class"
    Then I should see "Investigation: A Test Investigation"
  ...
  Scenario: Offerings from the default class show learner data in the default class
    ...
    And the following external activity exists:
      | name                 | user    | url    |
      | My External Activity | teacher | /about |
    When I login as an admin
    And I am on the class page for "Default Class"
    And I assign the external activity "My External Activity" to the class "Default Class"
    Then the external activity offering "My External Activity" in the class "Default Class" should be a default offering
    And I am logged in with the username teacher
    When I am on the class page for "My Class"
    And I assign the external activity "My External Activity" to the class "My Class"
    Then the external activity offering "My External Activity" in the class "My Class" should not be a default offering
    And the external activity named "My External Activity" should have "offerings_count" equal to "2"
    And I am logged in with the username student
    And I am on the class page for "My Class"
    And I follow "run My External Activity"
    Then I should be on the about page
    And I login as an admin
    And I am on the class page for "Default Class"
    Then I should see "My External Activity"
    And I should see "joe user"
    And the learner count for the external activity "My External Activity" in the class "Default Class" should be "1"
